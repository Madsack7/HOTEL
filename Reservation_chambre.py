...
def callback(event):
    ...
    if selection:
        ...
        conn=pymysql.connect(
              ...
              db='hotel')
        cur=conn.cursor()
        cur.execute("select num_chambre,categorie from chambre where num_chambre="+selection['2']+";")
        l=[]
        output=cur.fetchall()
        for i in output:
          a=str(i[0])+" "+i[1]
...
lblchambre = Label(root, text="Chambre", font=("sans serif", 12), background='#60554F', foreground='#FFFFFF')
lblchambre.place(x=30, y=130, width=80)
txtchambre=customtkinter.CTkComboBox(master=root,height=30,values=listechambre(),width=40,border_width=1,corner_radius=20,fg_color="#FFFFFF")
txtchambre.place(x=200,y=130,width=160)
...
id_rec=Entry(root,width=100,textvariable=id)
...
def enregistrer():
    chambre_=txtchambre.get().split(' ',-1)
    num_chambre=int(chambre_[0])
    ...
    date=date_.get()
    prix=txtdprix.get()
    print(num_chambre,client_nom,client_prenom,client_telephone,date,prix)
    try:
        if prix=="":
...
def modifier():
    ...
    try:
        conn=pymysql.connect(
              ...
              db='hotel')
        cur=conn.cursor()
        cur.execute("update reservation_chambre set num_chambre=%s, nom_client=%s,prenom_client=%s,telephone=%s,date_reservation=%s,prix=%s where id=%s;",(num_chambre,client_nom,client_prenom,client_telephone,date,prix,id))
        conn.commit()
...
def supprimer():
    ...
    if id_rec.get()=="":
        ...
        try:
            ...
            cur=conn.cursor()
            cur.execute("delete from reservation_chambre where id=%s;",id)
            conn.commit()
            dernier=cur.lastrowid
...
def rechercher():
    ...
    cur=conn.cursor()
    cur.execute("select id,num_chambre,nom_client,prenom_client,telephone,date_reservation,prix from reservation_chambre where "+str(type_rechercher.get())+" LIKE '%"+str(txtrechercher.get())+"%' ;")
...
b4 = Button(text='<-',font=("Arial",16),background='#FFFFFF',width=5,command=chambre)
b4.place(x=0, y=30)
...
def afficher():
    conn=pymysql.connect(
              ...
              db='hotel')
    cur=conn.cursor()
    cur.execute("select id,num_chambre,nom_client,prenom_client,telephone,date_reservation,prix from reservation_chambre;")
    tableAfficher.delete(*tableAfficher.get_children())
    output=cur.fetchall()
    for i in output:
        tableAfficher.insert('',END,values=i)
